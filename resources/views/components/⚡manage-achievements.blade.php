...
new class extends Component
{
    ...
    #[Validate('required')]
    public $category = 'Award'; // 'Award', 'Recognition', 'Fellowship', 'Certification', 'Other'
    ...
    {
        $this->reset(['title', 'description', 'year', 'category', 'issuing_body', 'link_url', 'link_label', 'is_featured', 'order', 'isEditing', 'editingId']);
        $this->year = date('Y');
        $this->category = 'Award';
    }
};
...
<div>
    ...
    @if($isEditing)
    <div class="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm mb-12" x-data="{ hasLink: @entangle('link_url') }">
        <form wire:submit="save" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label class="text-xs font-bold text-navy uppercase tracking-widest">Achievement/Award Title</label>
                    <input wire:model="title" type="text" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary outline-none" placeholder="e.g. Outstanding Medical Contribution Award" />
                    @error('title') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>
                <div class="space-y-2">
                    <label class="text-xs font-bold text-navy uppercase tracking-widest">Issuing Body (Optional)</label>
                    <input wire:model="issuing_body" type="text" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary outline-none" placeholder="e.g. World Health Organization" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="space-y-2">
                    ...
                    <select wire:model="category" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary outline-none">
                ...
                <div class="flex items-center gap-2 pt-6">
                    ...
                    <label for="is_featured" class="text-xs font-bold text-navy uppercase tracking-widest cursor-pointer">Featured (Pin to Top)</label>
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-xs font-bold text-navy uppercase tracking-widest">Description / Written Details</label>
                <textarea wire:model="description" rows="4" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary outline-none resize-none" placeholder="Provide background information, the significance, and written details about this award..."></textarea>
            ...
            <div class="border-t border-slate-100 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input wire:model="link_url" type="url" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary outline-none" placeholder="https://example.com/certificate-or-news-source" />
                        @error('link_url') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-navy uppercase tracking-widest">Link Label (What readers click)</label>
                        <input wire:model="link_label" type="text" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary outline-none" placeholder="e.g. Read News Release or View Credential" />
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200">
                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Title & Issuing Body</th>
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2.5 py-1 text-[10px] font-bold rounded-full uppercase tracking-widest
                            @if($item->category === 'Award') bg-amber-50 text-amber-700 border border-amber-200/50
                            @elseif($item->category === 'Recognition') bg-blue-50 text-blue-700 border border-blue-200/50
                            @elseif($item->category === 'Fellowship') bg-purple-50 text-purple-700 border border-purple-200/50
                            @elseif($item->category === 'Certification') bg-green-50 text-green-700 border border-green-200/50
                    ...
                    <td class="px-6 py-4">
                        @if($item->link_url)
                            <a href="{{ $item->link_url }}" target="_blank" class="inline-flex items-center gap-1 text-xs text-primary hover:underline font-semibold">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            ...
                            <button wire:confirm="Are you sure you want to delete this achievement?" wire:click="delete({{ $item->id }})" class="p-2 text-slate-400 hover:text-red-500 transition-colors">
